...
Sum of distances between each gps points
```{r}
df2 <- df %>% select(lon, lat)
df2$dist <- c(0, geosphere::distHaversine(head(df2,-1), tail(df2,-1)))


df <- df %>% mutate(dist=sum(df2$dist))
```
...
ggplot(df,aes(lon, lat))+
  ...
  ggrepel::geom_text_repel(data = filter(df, df$time==max(df$time)), 
    ...
    ggrepel::geom_text_repel(data = filter(df, df$time==min(df$time)), 
                             ...
                             nudge_x = -0.01,
                             size=3.5)+
  labs(title=paste0(round(mean(df$dist)),"m in ",round(difftime(max(df$time),min(df$time)))," min."))+
  theme(plot.background = element_rect(color = "black", fill = "black"),text = element_text(color="white"),plot.margin = unit(c(0,1,0,1),"cm"), legend.position = "bottom",plot.title = element_text(hjust=0.5))
